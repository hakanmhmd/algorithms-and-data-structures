...
public class ArrangementOfIntegerArray {
     ...
     *
     ...
     One way to see this is, we need to have a difference of \text{n-1}n-1, which means we need \text{1}1 and
     \text{n}n adjacent; then, we need a difference of \text{n-2}n-2, etc.

     Also, when \text{k = 1}k = 1, a valid construction is \text{[1, 2, 3, ..., n]}[1, 2, 3, ..., n].
     So we have a construction when \text{n-k}n-k is tiny, and when it is large. This leads to the idea
     that we can stitch together these two constructions: we can put \text{[1, 2, ..., n-k-1]}[1, 2, ..., n-k-1]
     first so that \text{n}n is effectively \text{k+1}k+1, and then finish the construction with the first
     \text{"k = n-1"}"k = n-1" method.

     For example, when \text{n = 6}n = 6 and \text{k = 3}k = 3, we will construct the array as
     \text{[1, 2, 3, 6, 4, 5]}[1, 2, 3, 6, 4, 5]. This consists of two parts: a construction of \text{[1, 2]}[1, 2]
     and a construction of \text{[1, 4, 2, 3]}[1, 4, 2, 3] where every element had \text{2}2 added to it
     (i.e. \text{[3, 6, 4, 5]}[3, 6, 4, 5]).
     */
